...
statuses: any;
  @Input() set tasks(value: Task[]) { // Use a setter to respond to input changes
    this.dataSource = new MatTableDataSource<Task>(value);
    if (this.matPaginator) { // If paginator is already set, attach it to the new data source
      this.dataSource.paginator = this.matPaginator;
    }
  }
  dataSource = new MatTableDataSource<Task>(); // Initialize dataSource

  @ViewChild(MatPaginator) set matPaginator(mp: MatPaginator) { // Use a setter for the paginator
